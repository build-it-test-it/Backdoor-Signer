...
final class CustomAIService {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            ModelFileManager.shared.prepareMLModel { [weak self] result in
                ...
                case let .success(modelURL):
                    ...
                    CoreMLManager.shared.loadModel { success in
                        if success, !(self?.isCoreMLInitialized ?? false) {
                            ...
                            Debug.shared.log(
                                message: "CoreML model successfully initialized via backup path",
                                type: .info
                            )
                        } else if !success {
                            Debug.shared.log(
                                message: "CoreML model failed to initialize, falling back to pattern matching",
                                type: .warning
                            )
                ...
                case let .failure(error):
                    Debug.shared.log(
                        message: "Failed to prepare ML model: \(error.localizedDescription), falling back to pattern matching",
                        type: .error
                    )
                    self?.isCoreMLInitialized = false
                }
    ...
    @objc private func handleCoreMLModelLoaded() {
        if !isCoreMLInitialized {
            isCoreMLInitialized = true
            Debug.shared.log(
                message: "CoreML model loaded notification received, enabling ML capabilities",
                type: .info
            )
        }
    }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            for pattern in searchCommandPatterns {
                if let range = lastUserMessage.range(of: pattern, options: .regularExpression) {
                    if let queryRange = lastUserMessage.range(
                        of: "\\[\\w+ search:([^\\]]+)\\]",
                           ...
                           let endBracketIndex = queryMatch.lastIndex(of: "]")
                        {
                            ...
                            let query = String(queryMatch[startIndex ..< endBracketIndex])
        ...
        for pattern in patterns {
               ...
               let queryRange = response.range(of: "\\[\\w+ search:([^\\]]+)\\]", options: .regularExpression)
            {
                   ...
                   let endBracketIndex = command.lastIndex(of: "]")
                {
                    ...
                    let query = String(command[startIndex ..< endBracketIndex])
                        .trimmingCharacters(in: .whitespacesAndNewlines)
                    ...
                    if command.contains("web search") {
                        performWebSearch(query: query) { searchResult in
                            completion(cleanedResponse + "\n\n" + searchResult)
                        }
                        return
                    } else if command.contains("deep search") {
                        let depth: SearchDepth = command.contains("specialized") ? .specialized : .deep
                        performDeepSearch(query: query, depth: depth) { searchResult in
                            completion(cleanedResponse + "\n\n" + searchResult)
                        }
                        return
                    } else if command.contains("academic search") {
                        performAcademicSearch(query: query) { searchResult in
                            completion(cleanedResponse + "\n\n" + searchResult)
                        }
                        return
                    } else if command.contains("news search") {
                        performNewsSearch(query: query) { searchResult in
                            completion(cleanedResponse + "\n\n" + searchResult)
                        }
                        return
                    }
    ...
    func analyzeUserIntent(message: String) -> MessageIntent {
        ...
        if lowercasedMessage.contains("help") || lowercasedMessage.contains("how do i") || lowercasedMessage
            .contains("how to")
        {
            return .generalHelp
        }
        ...
        if let match = lowercasedMessage.range(
            of: "(?:search|google|look up|find)\\s+(?:for\\s+)?(?:information\\s+about\\s+)?([^?.,]+)",
            options: .regularExpression
        ) {
            let query = String(lowercasedMessage[match]).replacing(
                regularExpression: "(?:search|google|look up|find)\\s+(?:for\\s+)?(?:information\\s+about\\s+)?",
                with: ""
            ).trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        if let match = lowercasedMessage.range(
            ...
            let appName = String(lowercasedMessage[match]).replacing(
        ...
        if let match = lowercasedMessage.range(
            ...
            let destination = String(lowercasedMessage[match]).replacing(
        ...
        if let match = lowercasedMessage.range(
            of: "add\\s+(?:a\\s+)?(?:new\\s+)?source\\s+(?:with\\s+url\\s+|at\\s+|from\\s+)?([^?]+)",
            options: .regularExpression
        ) {
            let url = String(lowercasedMessage[match]).replacing(
                regularExpression: "add\\s+(?:a\\s+)?(?:new\\s+)?source\\s+(?:with\\s+url\\s+|at\\s+|from\\s+)?",
                with: ""
            ).trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        if lowercasedMessage.contains("?") {
            ...
            let topic = lowercasedMessage.replacing(
    ...
    func generateResponse(
        ...
        case .unknown:
            ...
            if containsAppKeywords {
                return """
                I understand you need assistance with Backdoor. Based on your current context (\(
                    contextInfo
