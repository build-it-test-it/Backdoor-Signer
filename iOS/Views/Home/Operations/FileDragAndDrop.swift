...
extension HomeViewController {
    ...
    func handleExternalDrop(_ dropItem: UITableViewDropItem, coordinator _: UITableViewDropCoordinator) {
        ...
        if dropItem.dragItem.itemProvider.hasItemConformingToTypeIdentifier(UTType.url.identifier) {
            dropItem.dragItem.itemProvider
                .loadItem(forTypeIdentifier: UTType.url.identifier, options: nil) { [weak self] urlData, _ in
        ...
        if dropItem.dragItem.itemProvider.hasItemConformingToTypeIdentifier(UTType.image.identifier) {
            dropItem.dragItem.itemProvider
                .loadItem(forTypeIdentifier: UTType.image.identifier, options: nil) { [weak self] imageData, _ in
        ...
        if dropItem.dragItem.itemProvider.hasItemConformingToTypeIdentifier(UTType.plainText.identifier) {
            dropItem.dragItem.itemProvider
                .loadItem(forTypeIdentifier: UTType.plainText.identifier, options: nil) { [weak self] textData, _ in
                    ...
                    DispatchQueue.main.async {
                        ...
                        let tempURL = FileManager.default.temporaryDirectory
                            .appendingPathComponent("dropped_text_\(UUID().uuidString).txt")
                        do {
    ...
    func tableView(_: UITableView, dropSessionDidUpdate session: UIDropSession,
